...
final class VideoAnalyticsRepository
{
    ...
    public function adminOverview(): array
    {
        ...
        try {
            $statement = $pdo->query(
                'SELECT
                    ...
                    SUM(CASE WHEN viewed_on >= CURDATE() - INTERVAL 29 DAY THEN 1 ELSE 0 END) AS views_30d,
                    COUNT(DISTINCT CASE WHEN viewed_on >= CURDATE() - INTERVAL 29 DAY THEN session_key ELSE NULL END) AS unique_viewers_30d,
                    COUNT(DISTINCT CASE WHEN viewed_on >= CURDATE() - INTERVAL 29 DAY THEN video_id ELSE NULL END) AS active_videos_30d,
                    COUNT(DISTINCT CASE WHEN viewed_on >= CURDATE() - INTERVAL 29 DAY THEN creator_user_id ELSE NULL END) AS active_creators_30d
                 FROM video_views'
            );
            $row = $statement->fetch() ?: [];
        } catch (Throwable) {
        ...
        return [
            ...
            'views_30d' => (int) ($row['views_30d'] ?? 0),
            'unique_viewers_30d' => (int) ($row['unique_viewers_30d'] ?? 0),
            'active_videos_30d' => (int) ($row['active_videos_30d'] ?? 0),
            'active_creators_30d' => (int) ($row['active_creators_30d'] ?? 0),
        ];
    }
     ...
     * @return array<int, array{date:string,label:string,views:int}>
     */
    public function adminDailySeries(int $days = 14): array
    {
        $pdo = Database::connection();
        $days = max(1, min(90, $days));
        $end = new DateTimeImmutable('today');
        ...
        try {
            $statement = $pdo->prepare(
                'SELECT viewed_on, COUNT(*) AS total
                 FROM video_views
                 WHERE viewed_on BETWEEN :start_date AND :end_date
                 GROUP BY viewed_on
                 ORDER BY viewed_on ASC'
            );
            $statement->execute([
     ...
     * @return array<int, array<string, mixed>>
     */
    public function adminTopVideos(int $limit = 12): array
    {
        $pdo = Database::connection();
        ...
        try {
            $statement = $pdo->prepare(
                'SELECT
                 ...
                 GROUP BY videos.id, videos.slug, videos.title, videos.creator_name, videos.creator_user_id, videos.access_level, videos.moderation_status, videos.published_at
                 ORDER BY total_views DESC, views_30d DESC, videos.created_at DESC
                 LIMIT :limit'
            );
            $statement->bindValue(':limit', $limit, PDO::PARAM_INT);
     ...
     * @return array<int, array<string, mixed>>
     */
    public function topCreators(int $limit = 10): array
    {
        $pdo = Database::connection();
        ...
        try {
            $statement = $pdo->prepare(
                'SELECT
                    users.id,
                    users.display_name,
                    users.creator_display_name,
                    users.creator_slug,
                    COUNT(video_views.id) AS total_views,
                    SUM(CASE WHEN video_views.viewed_on >= CURDATE() - INTERVAL 29 DAY THEN 1 ELSE 0 END) AS views_30d,
                    COUNT(DISTINCT video_views.video_id) AS videos_with_views,
                    MAX(video_views.created_at) AS last_view_at
                 FROM video_views
                 INNER JOIN users ON users.id = video_views.creator_user_id
                 GROUP BY users.id, users.display_name, users.creator_display_name, users.creator_slug
                 ORDER BY total_views DESC, views_30d DESC, last_view_at DESC
                 LIMIT :limit'
            );
            $statement->bindValue(':limit', $limit, PDO::PARAM_INT);
            $statement->execute();
        ...
        return array_map(static function (array $row): array {
            ...
            return [
                'id' => (int) ($row['id'] ?? 0),
                'name' => creator_public_name($creator, (string) ($row['display_name'] ?? 'Creator')),
                'creator_slug' => (string) ($row['creator_slug'] ?? ''),
                'channel_url' => creator_profile_url($creator),
                'total_views' => (int) ($row['total_views'] ?? 0),
                'views_30d' => (int) ($row['views_30d'] ?? 0),
                'videos_with_views' => (int) ($row['videos_with_views'] ?? 0),
                'last_view_at' => $lastViewAt,
                'last_view_label' => format_datetime($lastViewAt),
            ];
        }, $rows);
