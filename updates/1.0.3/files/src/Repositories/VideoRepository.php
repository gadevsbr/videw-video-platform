...
final class VideoRepository
{
    private const SELECT_COLUMNS = 'videos.id, videos.slug, videos.title, videos.synopsis, videos.creator_user_id, videos.creator_name,
        ...
        videos.moderation_reason, videos.moderation_notes, videos.published_at, videos.deleted_at, videos.created_at, videos.updated_at';
        ...
        try {
            $statement = $pdo->prepare(
                'INSERT INTO videos (
                    ...
                    moderation_reason, moderation_notes, published_at, deleted_at, created_at, updated_at
                ) VALUES (
                    :slug, :title, :synopsis, :creator_user_id, :creator_name, :category, :access_level, :duration_minutes, :poster_tone,
                    :poster_url, :poster_path, :poster_storage_provider, :poster_focus_x, :poster_focus_y, :video_url, :file_path, :trailer_url, :embed_url,
                    :mime_type, :original_source_url, :source_type, :storage_provider, :is_featured, :moderation_status,
                    :moderation_reason, :moderation_notes, :published_at, NULL, NOW(), NOW()
                )'
            );
            $statement->execute([
                'slug' => $payload['slug'],
    ...
    public function updateModeration(int $id, string $status, ?string $reason = null, ?string $notes = null): void
    {
        $pdo = Database::connection();
        ...
        try {
            ...
            $statement->execute([
                'id' => $id,
                'status' => $status,
                'reason' => normalize_moderation_reason($reason),
                'notes' => $notes,
     ...
     * @param array<int, int> $ids
     */
    public function bulkUpdateModeration(array $ids, string $status, ?string $reason = null, ?string $notes = null): int
    {
        $count = 0;

        foreach (array_unique(array_filter(array_map('intval', $ids), static fn (int $id): bool => $id > 0)) as $id) {
            $this->updateModeration($id, $status, $reason, $notes);
            $count++;
        }
    ...
    private function buildAdminWhere(array $filters): array
    {
        ...
        $reason = normalize_moderation_reason((string) ($filters['reason'] ?? ''));
        ...
        if ($reason !== '') {
            $conditions[] = 'moderation_reason = :reason';
    ...
    private function normalizeVideo(array $video): array
    {
        ...
        return [
            ...
            'moderation_reason' => normalize_moderation_reason((string) ($video['moderation_reason'] ?? '')),
            'moderation_reason_label' => moderation_reason_label((string) ($video['moderation_reason'] ?? '')),
