 ...
 * @return array<string, string>
 */
function moderation_reason_options(): array
{
    return [
        '' => 'No structured reason',
        'ready_to_publish' => 'Ready to publish',
        'metadata_fix' => 'Needs metadata fix',
        'media_fix' => 'Needs media fix',
        'policy_review' => 'Needs policy review',
        'copyright_risk' => 'Possible copyright risk',
        'duplicate_or_spam' => 'Duplicate or spam',
        'manual_hold' => 'Manual hold',
    ];
}

function normalize_moderation_reason(?string $reason): string
{
    $reason = trim((string) $reason);

    return array_key_exists($reason, moderation_reason_options()) ? $reason : '';
}
...
function app_settings_to_env_values(array $settings): array
{
    return [
        ...
        'VIDEW_GITHUB_REPOSITORY' => (string) ($settings['github_repository'] ?? config('updates.github_repository', '')),
        'VIDEW_APP_VERSION' => (string) ($settings['current_version'] ?? config('updates.current_version', '')),
