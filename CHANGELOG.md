...
### Added
- Added GitHub release monitoring in the admin workspace, including repository/version settings, latest-release lookup, cached API checks, and an overview card with update visibility.
- Added a JSON-based admin backup export covering app, storage, billing, legal, copy, and ad-slot settings so operators can download a configuration snapshot from the admin panel.
- Added local Stripe webhook event tracking with duplicate-event protection, retry-aware state files, and admin-facing webhook diagnostics.
- Added manual retry support for failed Stripe webhook events, plus recent webhook history and a failed-event queue directly in the admin billing workspace.
- Added admin export tooling for catalog and users, including JSON and CSV downloads from both the filtered workspace screens and the full-settings export area.
- Added an admin analytics screen with aggregate view metrics, 14-day trend data, top videos, and top creators powered by the local `video_views` table.
- Added structured moderation reasons, a `updates/1.0.3/sql/002-video-moderation-reasons.sql` upgrade, reason-based moderation filtering, and recent per-video moderation history inside the admin queue.
- Added database schema version tracking through a `schema_migrations` table, a `updates/1.0.3/sql/001-schema-version-tracking.sql` upgrade, and a runtime service that compares code version versus installed DB version.
- Added a local `docs/ROADMAP_STATUS.md` working document to map roadmap items against the current implementation status and priority order.
- Added `docs/BACKUP_RESTORE.md` to document how admin exports fit into full backup and restore workflows for existing installations.
...
- Improved platform visibility for operators by giving the admin area a dedicated analytics workspace instead of limiting traffic reporting to creator-facing screens.
- Improved moderation workflow with preset reasons, queue filtering by reason, and clearer audit-backed review context on each moderated item.
- Improved the admin `Settings` screen with release-monitoring controls and a direct backup-download action.
- Improved the admin `Overview` screen by surfacing both release-monitoring status and more operational webhook health context.
- Improved the admin workspace with database version visibility so operators can see the current app version, tracked DB version, latest migration entry, and whether an upgrade is still pending.
- Improved the installer defaults so new installs seed `VIDEW_APP_VERSION` from the current package version instead of relying on a stale manual value.

### Fixed
- Fixed Stripe webhook handling so repeated deliveries of the same event no longer trigger the same processing path multiple times.
- Fixed GitHub release checks on local WAMP-style Windows environments by resolving a CA bundle automatically when `curl.cainfo` or `openssl.cafile` is not configured.
